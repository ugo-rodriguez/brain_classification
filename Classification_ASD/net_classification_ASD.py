...
class BrainNet(pl.LightningModule):
    def __init__(self,nbr_features,dropout_lvl,image_size,noise_lvl,ico_lvl,batch_size,weights, radius=1.35, lr=1e-4,name=''):
        ...
        self.train_accuracy = torchmetrics.Accuracy('binary',num_classes=2)
        self.val_accuracy = torchmetrics.Accuracy('binary',num_classes=2,average='weighted')#,average='weighted'
    ...
    def GetView(self,meshes,index):
        ...
        R = self.R[index][None].to(self.device,non_blocking=True)
        T = self.T[index][None].to(self.device,non_blocking=True)
