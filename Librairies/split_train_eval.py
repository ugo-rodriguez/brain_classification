...
if args.group_by:
    ...
    samples_val = int(len(group_ids)*0.1) ###
    ...
    if args.folds == 0:
        ...
        for i in range(args.folds):
            ...
            id_train_and_val = np.setdiff1d(group_ids, id_test) #
            id_val = id_train_and_val[0:samples_val] #

            df_val = df_train[df_train[args.group_by].isin(id_val)] #
            df_train = df_train[~df_train[args.group_by].isin(id_val)] #
            ...
            if(os.path.splitext(fname)[1] == ".csv"):
                ...
                val_fn = fname.replace('.csv', 'fold' + str(i) + '_val.csv') #
                df_val.to_csv(val_fn, index=False) #
